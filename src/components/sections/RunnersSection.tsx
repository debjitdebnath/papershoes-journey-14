...
const RunnerCard = ({ runner, index }: RunnerCardProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden">
        <motion.img
          src={runner.image}
          alt={`Portrait of ${runner.name}`}
          className="w-full h-full object-cover object-top filter grayscale-[30%] transition-all duration-500"
          animate={{
      ...
      <div className="absolute top-4 left-4 z-10">
        <p className="text-cream/70 text-xs font-light tracking-widest uppercase">
          {runner.country}
        </p>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 z-10">
        ...
        {/* Quote */}
        <motion.p
          className="text-cream/70 text-sm italic leading-relaxed mb-6 transition-opacity duration-300"
        ...
        <motion.div
          ...
          style={{ opacity: 'var(--social-opacity, 0)' }}
        >
          {runner.socials.instagram && (
            <a
              href={runner.socials.instagram}
              className="text-cream/60 hover:text-terracotta transition-colors duration-200 p-2 -m-2"
              aria-label={`${runner.name}'s Instagram`}
              target="_blank"
          ...
          <a
            href="#"
            className="text-cream/60 hover:text-terracotta transition-colors duration-200 p-2 -m-2"
            aria-label={`${runner.name}'s X (Twitter)`}
            target="_blank"
            rel="noopener noreferrer"
          >
            <XIcon />
          </a>
          {runner.socials.linkedin && (
            <a
              href={runner.socials.linkedin}
              className="text-cream/60 hover:text-terracotta transition-colors duration-200 p-2 -m-2"
              aria-label={`${runner.name}'s LinkedIn`}
              target="_blank"
              rel="noopener noreferrer"
            >
          ...
          {runner.socials.email && (
            <a
              href={`mailto:${runner.socials.email}`}
              className="text-cream/60 hover:text-terracotta transition-colors duration-200 p-2 -m-2"
              aria-label={`Email ${runner.name}`}
            >
              <Mail className="w-5 h-5" />
            </a>
          )}
          {runner.socials.phone && (
            <a
              href={`tel:${runner.socials.phone}`}
              className="text-cream/60 hover:text-terracotta transition-colors duration-200 p-2 -m-2"
              aria-label={`Call ${runner.name}`}
            >
...
const RunnersSection = () => {
  ...
  return (
    <section className="bg-earth-900 py-20 md:py-28 overflow-hidden" id="runners">
        ...
        <div className="md:hidden">
          <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 pb-6 -mx-4 px-4 scrollbar-hide">
            {runners.map((runner, index) => (
              <div
                key={runner.id}
                className="snap-center flex-shrink-0 w-[85%] max-w-[340px]"
