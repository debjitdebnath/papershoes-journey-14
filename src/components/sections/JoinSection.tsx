...
import { Users, School, Building2, Newspaper, ClipboardList, ArrowUpRight } from 'lucide-react';
    ...
    <section id="join" className="py-16 md:py-20 bg-background" ref={ref}>
      <div className="container mx-auto px-6">
        ...
        >
          ...
          <p className="inline-flex items-center gap-2 text-sm text-muted-foreground bg-muted px-4 py-2 rounded-full">
            <ClipboardList className="w-4 h-4 text-sage" />
            Pick a path below — each one opens a short sign-up form.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 max-w-5xl mx-auto">
          {pathways.map((p, i) => (
            <motion.a
              ...
              className="group bg-card rounded-2xl p-5 shadow-soft hover:shadow-card hover:-translate-y-1 transition-all border border-border flex flex-col"
            >
              ...
              <span className="mt-auto inline-flex items-center gap-1 text-xs font-semibold text-sage-dark">
                Fill the form <ArrowUpRight className="w-3.5 h-3.5" />
              </span>
            </motion.a>
          ))}
        </div>
